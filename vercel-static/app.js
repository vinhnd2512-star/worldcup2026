...
function renderMatches() {
  const featured = state.matches.find((match) => match.status === "SCHEDULED") || state.matches[0];
  if (!featured) {
    return `<section class="glass-card panel"><h2>Chưa có lịch đấu</h2><p>Hãy chạy Supabase seed.</p></section>`;
  }
  const upcoming = state.matches.filter((match) => match.status === "SCHEDULED" && match.id !== featured.id).slice(0, 4);
  ...
  return `
    <div class="dashboard-grid">
      <div class="stack">
        ...
        <div class="section-heading"><h2>Trận đấu sắp tới</h2><span>${upcoming.length + 1} trận mở kèo</span></div>
        <section class="card-grid">${upcoming.map(renderMatchCard).join("")}</section>
      </div>
      <aside class="stack">
...
function renderMatchCard(match) {
  ...
  return `
    <article class="match-card glass-card">
      <div class="section-heading"><span>${dateText(match.starts_at)}</span><span>x${fmtOne.format(number(odd))}</span></div>
...
async function syncProviders() {
  ...
  if (!response.ok) {
    ...
    state.message = `Provider sync finished. Fixtures: ${fixtureStatus}; stats: ${statsStatus}; odds: ${oddsStatus}.`;
    state.error = "";
  }
  await loadData();
}
